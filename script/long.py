...
"""
Long entry script.
Places a limit buy order, waits for fill, falls back to market order.
Records the open position in DB.
Called by Claude Code via the /long skill or directly.
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [LONG] %(message)s",
)
...
def run():
    ...
    if settings.dry_run:
        ...
        print(f"LONG recorded (DRY RUN): entry_price={mid} trade_id={trade_id}")
        return
    ...
    logger.info(f"Placing LONG limit: {coin} qty={qty} (sz_decimals={sz_decimals}) px={limit_px}")
    ...
    if not filled and oid:
        ...
        if not filled:
            ...
            time.sleep(1)
            market_result = exchange.market_open(coin, True, qty, None, slippage=0.01)
            statuses = (
