...
**4a. `## 学習済みルール` セクション（禁止・注意ルール）**
- 今回の結果から「〇〇の時はエントリー禁止」「〇〇に注意」という禁止・警戒ルールがあれば追記・更新
- 最大10件を維持（超える場合は重複・陳腐化したルールを統合または削除）

**4b. `## エントリー推奨条件` セクション（ポジティブルール）**
- {result_label} トレードの**勝因・有効だったシグナル**を「〇〇の時はエントリー検討」という形で1件抽出してください
- LOSSの場合でも「このシグナルが揃っていればWINだった可能性がある」という仮説的ポジティブ条件を1件記録してください
- `## エントリー推奨条件` セクションが存在しない場合は新規作成してください
- 最大5件を維持（超える場合は重複・陳腐化したものを統合または削除）
- フォーマット例: `- **条件名**: 〇〇足が△△の状態で□□が確認できる場合はロング/ショートを検討`
