...
ステップ4: `/app/AGENTS.md` をReadツールで読み込み、以下の2セクションをEditツールで更新してください。
AGENTS.md はHTMLで記述されています。Markdownではなく正しいHTMLタグを使用してください。

**4a. `<h2>学習済みルール</h2>` セクション（禁止・注意ルール）**
- 今回の結果から「〇〇の時はエントリー禁止」「〇〇に注意」という禁止・警戒ルールがあれば `<li>` として追記・更新
- `<ol>` タグ内の `<li>` を編集すること
- 最大10件を維持（超える場合は重複・陳腐化したルールを統合または削除）
- フォーマット: `<li><strong>ルール名</strong>: 説明文</li>`

**4b. `<h2>エントリー推奨条件</h2>` セクション（ポジティブルール）**
- {result_label} トレードの勝因・有効だったシグナルを `<li>` として1件抽出してください
- LOSSの場合でも「このシグナルが揃っていればWINだった可能性がある」という仮説的ポジティブ条件を1件記録してください
- `<h2>エントリー推奨条件</h2>` セクションが存在しない場合は `<ol></ol>` ごと新規作成してください
- 最大5件を維持（超える場合は重複・陳腐化したものを統合または削除）
- フォーマット: `<li><strong>条件名</strong>: 〇〇足が△△の状態で□□が確認できる場合はロング/ショートを検討</li>`
